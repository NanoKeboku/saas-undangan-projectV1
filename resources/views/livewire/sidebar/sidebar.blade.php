<aside 
    ...
    <nav class="flex-1 space-y-6 overflow-y-auto custom-scrollbar overflow-x-hidden pr-2">
        <div>
            ...
            <div class="space-y-1">
                <a href="{{ route('dashboard') }}" wire:navigate 
                   class="group relative flex items-center gap-4 px-5 py-3 rounded-2x transition-all duration-300 {{ request()->routeIs('dashboard') ? 'bg-[#99B9FF]/20 text-[#2159D4]' : 'text-[#6B6B6B] hover:bg-[#F8FAFC] hover:text-black' }}">
        ...
        <div>
            <p x-show="expanded" class="px-5 mb-2 text-[10px] uppercase tracking-widest text-gray-400">Management</p>
            <div class="space-y-1">
                
                <div x-data="{ open: {{ request()->routeIs('my-theme.*') ? 'true' : 'false' }} }">
                    <button @click="expanded ? (open = !open) : (expanded = true, open = true)" 
                        class="w-full flex items-center justify-between px-5 py-3 rounded-2xl transition-all {{ request()->routeIs('my-theme.*') ? 'text-[#2159D4]' : 'text-[#6B6B6B] hover:bg-[#F8FAFC]' }}">
                ...
                <div x-data="{ open: {{ request()->routeIs('tamu.*') ? 'true' : 'false' }} }">
                    <button @click="expanded ? (open = !open) : (expanded = true, open = true)" 
                        class="w-full flex items-center justify-between px-5 py-3 rounded-2xl transition-all {{ request()->routeIs('tamu.*') ? 'text-[#2159D4]' : 'text-[#6B6B6B] hover:bg-[#F8FAFC]' }}">
                        <div class="flex items-center gap-4 text-left">
                ...
                <div x-data="{ open: {{ request()->routeIs('egift.*') ? 'true' : 'false' }} }">
                    <button @click="expanded ? (open = !open) : (expanded = true, open = true)" 
                        class="w-full flex items-center justify-between px-5 py-3 rounded-2xl transition-all {{ request()->routeIs('egift.*') ? 'text-[#2159D4]' : 'text-[#6B6B6B] hover:bg-[#F8FAFC]' }}">
                        <div class="flex items-center gap-4 text-left">
                    ...
                    <div x-show="open && expanded" x-collapse class="mt-1 ml-11 space-y-1 border-l-2 border-[#99B9FF]/30">
                        <a href="{{ route('egift.payout') }}" wire:navigate class="block px-6 py-2 text-sm  {{ request()->routeIs('egift.payout') ? 'text-[#2159D4]' : 'text-[#6B6B6B] hover:text-black' }}">Payout Gift</a>
                    </div>
                    <div x-show="open && expanded" x-collapse class="mt-1 ml-11 space-y-1 border-l-2 border-[#99B9FF]/30">
                        <a href="{{ route('egift.settings') }}" wire:navigate class="block px-6 py-2 text-sm  {{ request()->routeIs('egift.settings') ? 'text-[#2159D4]' : 'text-[#6B6B6B] hover:text-black' }}">Setting Gift</a>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div>
            <p x-show="expanded" class="px-5 mb-2 text-[10px]  uppercase tracking-widest text-gray-400">Settings & Help</p>
            <div class="space-y-1">
